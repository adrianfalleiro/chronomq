...
type Hub struct {
	...
	spokeMap  map[temporal.Bound]*Spoke // Quick lookup map
...
// findOwnerSpoke returns the spoke that owns this  Lock the hub before calling this
	...
	go func() {
		...
		func() {
			...
			for i := 0; i < pastJobsLen; i++ {
				jobChan <- h.pastSpoke.JobAtIdx(i)
				n--
				if n <= 0 {
					return
				}
