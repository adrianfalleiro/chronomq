...
// IndexedSpoke is a memory-efficient spoke that stores only job indices in memory
// while keeping the full job data on disk
type IndexedSpoke struct {
	id uuid.UUID
	temporal.Bound
...
// NewIndexedSpoke creates a new indexed spoke
func NewIndexedSpoke(start, end time.Time, diskStore persistence.DiskJobStore) *IndexedSpoke {
...
func (s *IndexedSpoke) IsJobInBounds(j *Job) bool {
...
func (s *IndexedSpoke) AddJobLocked(j *Job) error {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *IndexedSpoke) AddJobIndexLocked(idx *JobIndex) error {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *IndexedSpoke) NextLocked() (*Job, error) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *IndexedSpoke) CancelJobLocked(id string) (*Job, error) {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *IndexedSpoke) OwnsJobLocked(id string) bool {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *IndexedSpoke) MemoryFootprint() int {
	s.lock.Lock()
	defer s.lock.Unlock()
...
// PersistIndices persists all job indices (the jobs are already on disk)
func (s *IndexedSpoke) PersistIndices(p persistence.Persister) chan error {
	s.Lock()
	...
	go func() {
		...
		for i = 0; i < s.indexQueue.Len(); i++ {
			idx := s.JobIndexAtIdx(i)
			// For indices, we might want to persist the index metadata
			// For now, the actual jobs are already on disk via diskStore
			_ = idx // Jobs are already persisted via diskStore
		}

		log.Info().
			Int("indexCount", i).
			Str("spokeID", s.id.String()).
			Msg("Indexed spoke persistence completed (jobs already on disk)")
	}()
